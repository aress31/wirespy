    ...
    do
        ...
        case $AP_type in
            [1-2])
                check="success";;
            *) 
                echo -e "${WARNING}Type ${BOLD}Blackhole(1)${NORMAL}${WARNING} or ${BOLD}Bullzeye(2)${NORMAL}${WARNING}!\n${AQUA}"
                check="fail";;
        esac
    done
    check="fail"
    ...
    do
        ...
        then
            ...
            then
                ...
                echo -e "${WARNING}Type yes(y) or no(n)!"
                check="fail"
            fi
...
function show_intf()
{
    ...
    for intf in $(ip -o link show | awk -F': ' '{print $2}') # get the network interfaces names
    do
        case ${intf} in
            lo)
                continue;;
...
function show_w_intf()
{
    ...
    for intf in $(ip -o link show | awk -F': ' '{print $2}' | grep -e "wlan*") # get the interfaces names
    do
...
{
    if [[ $(ip link show $1 2> /dev/null) ]];
    then
        check="success"
    else
        echo -e "${WARNING}Device ${BOLD}$1 ${NORMAL}${WARNING}does NOT exist!"
        check="fail"
    fi
}  
